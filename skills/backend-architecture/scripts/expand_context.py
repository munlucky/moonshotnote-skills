#!/usr/bin/env python3
"""Write a compact source pack from backend architecture graph query results."""
...
def format_refs(refs: list[dict]) -> str:
    parts = []
    for ref in refs:
        parts.append(f"{ref.get('source_skill', '?')}:{ref.get('source_id', '?')}")
    return "; ".join(parts)
...
def build_pack(query: str, limit: int) -> str:
    ...
    adapter_text = (refs / "framework_adapters.yaml").read_text(encoding="utf-8")
    ...
    lines = [
        "# Backend Architecture Source Pack",
        "",
        f"Query: {query}",
        "",
        "## Nodes",
    ...
    if score_text(query_tokens, adapter_text):
        lines.extend(
            [
                "",
                "## Framework Adapter Note",
                "- FastAPI adapter mapping is available in references/framework_adapters.yaml.",
                "- Django, NestJS, and Spring are extension points only in v1.",
            ]
        )
    lines.extend(["", "Use this pack as public-safe context. Do not quote private OCR source text from upstream skills."])
    return "\n".join(lines) + "\n"

#!/usr/bin/env python3
"""Validate the public-safe FastAPI clean architecture graph."""
...
NODE_TYPES = {"Concept", "Layer", "Class", "API", "Pattern", "Warning", "SourceRef"}
EDGE_TYPES = {
    "depends_on",
    "implements",
    "belongs_to",
...
RAW_TEXT_RISK_PATTERNS = [
    ...
    re.compile(r"/Users/"),
    re.compile(r"\bpage_outputs\b", re.IGNORECASE),
]
...
def validate_graph(references_dir: Path) -> None:
    ...
    if quality.get("needs_review_remaining") != 0:
        raise ValueError("graph_manifest.json: needs_review_remaining must be 0")
    if not quality.get("reviewed_ocr_sha256"):
        raise ValueError("graph_manifest.json: reviewed_ocr_sha256 is required")
